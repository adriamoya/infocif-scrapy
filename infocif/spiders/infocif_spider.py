...
general_url = 'http://www.infocif.es/general/empresas-informacion-listado-empresas.asp?Buscar='
...
with open(os.path.dirname(__file__) + '/../inputs/cifs.csv', 'rb') as f:
    reader = csv.reader(f)
    for row in reader:
        start_urls_list.append(general_url+row[0])
	...
	def parse(self, response):
		...
		if response.status == 200:
			...
			if not not_exists:
				...
				# -------------------------------------------------
				try:
					sector = response.xpath('//strong[contains(text(),"Sector")]/following-sibling::*/text()')[0].extract().strip()
					if sector:
